function [newL2] = best_map(L1,L2)
    ...
    [c,t] = hungarian(-G);
    newL2 = zeros(size(L2));
    unmatchedBase = max(double(Label1))+1;
    for i=1:nClass2
        if c(i) <= nClass1
            target = Label1(c(i));
        else
            % A predicted cluster with no true-class assignment must remain
            % unmatched; indexing Label1(c(i)) used to fail here.
            target = unmatchedBase+i;
        end
        newL2(L2 == Label2(i)) = target;
    end
